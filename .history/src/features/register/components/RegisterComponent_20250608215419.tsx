...
const RegisterComponent = () => {
  ...
  // Observer setup to track scroll visibility
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (index === 0) setShowLeftArrow(!entry.isIntersecting);
          if (index === steps.length - 1) setShowRightArrow(!entry.isIntersecting);
        });
      },
      {
        root: container,
      ...
      <div className="w-full flex justify-between items-center px-10 mb-2">
        {showLeftArrow ? (
          <button
            ...
            className="bg-gradient-to-b from-[#CF5406] to-[#F87B18] text-white p-1.5 rounded-full shadow-sm"
