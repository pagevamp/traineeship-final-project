...
const RegisterComponent = () => {
  ...
  // Scroll stepper to current step (centered)
  useEffect(() => {
    ...
    if (currentStepElement && stepperContainerRef.current) {
      ...
      const offset =
        stepRect.left - containerRect.left - containerRect.width / 2 + stepRect.width / 2;
  ...
  // Actual step logic (only changed via Continue/Back)
  ...
  // Just scrolls the stepper container — does NOT change currStep
  ...
  // Step rendering (replace with your actual step components)
  const renderStep = () => {
    switch (currStep) {
      case 1:
    ...
    <section className="font-secondary max-w-[964px] mx-auto h-full flex flex-col items-center pt-[34px]">
      ...
      <div className="flex items-center w-full mb-6 relative">
        ...
        <button
          ...
          className="absolute left-0 z-10 rounded-full p-2 shadow bg-white hover:bg-gray-100"
        >
          <ChevronLeft size={24} />
        </button>
          ...
          <div className="flex gap-4 min-w-max">
            {steps.map((step, index) => (
              <div
                ...
                role="button"
                aria-current={currStep === step.id ? "step" : undefined}
              >
                <Stepper steps={[step]} currentStep={step.id} />
              </div>
            ))}
          </div>
        </div>
        ...
        <button
          ...
          className="absolute right-0 z-10 rounded-full p-2 shadow bg-white hover:bg-gray-100"
        >
