...
const DepartmentStatus = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "Pending" | "Rejected" | "Approved" | "Designation"
  >("Pending");

  const tabs = ["Pending", "Rejected", "Approved", "Designation"] as const;

  return (
    <div className="w-[1110px] flex flex-col gap-[15px]">
                  ...
                  >
                    ...
                    <TableCell className="py-[19px]">
                      <div
                        ...
                        role="button"
                        onClick={() =>
                          router.push(`/customer/CompanyInformation/${item.id}`)
                        }
                      >
                        <EyeIcon
                          size={23}
            ...
            <TableBody>
              {designations.map((title, index) => (
                <TableRow
                  ...
                  className="text-[#0B0704] text-[13px] font-secondary"
                >
                  <TableCell className="py-[19px]">{index + 1}</TableCell>
                  <TableCell className="py-[19px]">{title}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
